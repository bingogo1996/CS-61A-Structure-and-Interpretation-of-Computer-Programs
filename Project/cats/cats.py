...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    f = [i for i in paragraphs if select(i)]
    if k >= len(f):
        return ''
    else:
        return f[k]
...
def accuracy(typed, reference):
    ...
    if len(reference_words) == 0 and len(typed_words) == 0:
        ...
        return 0.0
    count = 0
    l = min(len(typed_words), len(reference_words))
    for i in range(l):
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    diff = [
        diff_function(typed_word, valid_words[i], limit)
...
def feline_flips(start, goal, limit):
    ...
    def diff_char(s1, s2, lim=limit):
        ...
        if len(s1) * len(s2) == 0:
            ...
            return diff_char(
                s1[1:], s2[1:], lim -
                (0 if s1[0] == s2[0] else 1)) + (0 if s1[0] == s2[0] else 1)

    return diff_char(start, goal)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0:
        ...
        substitute = diff + minimum_mewtations(start[1:], goal[1:],
                                               limit - diff)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(0, len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            print('ID:', user_id, 'Progress:', t / len(prompt))
            print(t / len(prompt))
            return None
    print('ID:', user_id, 'Progress:', t / len(prompt))
    print(t / len(prompt))
...
def time_per_word(words, times_per_player):
    ...
    for i in times_per_player:
        t = t + [[i[j + 1] - i[j] for j in range(len(i) - 1)]]
    return match(words, t)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    player_indices = range(len(
        get_times(match)))  # contains an *index* for each player
    ...
    for i in word_indices:
        time_min = time_min + [
            min([time(match, j, i) for j in player_indices])
        ]
    fast_word = list()
    for j in player_indices:
        fast_word = fast_word + [[
            word_at(match, i)
            for i in word_indices if time(match, j, i) == time_min[i]
            and not any([word_at(match, i) in k for k in fast_word])
        ]]
    return fast_word
    # END PROBLEM 10
